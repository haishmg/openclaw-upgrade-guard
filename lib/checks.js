...
export function evaluate({ commands, baseline, mode }) {
  ...
  for (const command of Object.values(commands)) {
    if (command.required && !command.ok) {
      ...
      const level = command.required || transportFailure ? "error" : "warning";
  ...
  evaluateGatewayStatus(commands.gateway_status?.json, add);
...
function evaluateGatewayStatus(gatewayStatus, add) {
  ...
  if (gatewayStatus.rpc?.ok === false) {
    add("error", "gateway.status.rpc", "Gateway status RPC probe failed", gatewayStatus.rpc);
